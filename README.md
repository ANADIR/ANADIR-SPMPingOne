...
1. The app with `PingID` should have the method `setKeepTrustedDeviceAfterReinstall` with value `true` before migrating to PingOne, otherwise migration will not run. In case this value is `false`, the device is considered as an untrusted device and will need pairing to `PingOne`, not migration.
2. Remove the `PingID_SDK.xcframework` framework from your project and any methods that call this SDK.
3. Add and set up the `PingOne` framework as described in the [Set up](#setup) section and implement SDK method as in [PingOne Mobile SDK sample app](#sample_app) sections above.
...
Add the `migrateFromPingID` method at the desired place in your app. For example, it can be called from `didFinishLaunchingWithOptions` method.
```swift
/// Migrate from PingID
///
...
**Note:** The `temporaryFailed`status is recoverable, user can try again. This can happen due to temporary error such as connectivity issue or timeout.
